...
const isNotClientLimit = function (ownerId) {
  const clients = Client.find({ ownerId });
...
const isUserSessionIdNew = function (userSessionId) {
  const chatMsgCurr = Chat.find({ userSessionId });
...
Meteor.methods({
  ...
  addClientApp(name) {
    ...
    if (this.userId && isNotClientLimit(this.userId)) {
      Client.insert({ name, ownerId: this.userId });
    }
  },
  ...
  removeChatMessages(clientAppId, selector, type) {
    ...
    if (type === 'clientIp') {
      const chatCurr = Chat.find({ clientAppId, clientIp: selector }, { fields: { userSessionId: 1 } }).fetch();
      chatCurr.forEach((msg) => {
        Chat.remove({ clientAppId, userSessionId: msg.userSessionId });
      });
    }
    if (type === 'sessionId') {
      Chat.remove({ clientAppId, userSessionId: selector });
    }
  },
  ...
  banUserIp(clientAppId, clientIp) {
    ...
    if (isProperIP(clientIp) && !Banned.findOne({ clientAppId, clientIp })) {
      Banned.insert({
        clientAppId,
        clientIp,
      });
